...
### 2. 用户模块 (user)
- **CartView.vue**: 购物车页面
  - 功能: 购物车商品列表展示、商品数量调整、商品选中状态管理、批量操作、积分统计
  - 状态: 已实现
  - 依赖: cart.js(store), cart.js(api)

- **FavoritesView.vue**: 收藏夹页面
  - 功能: 用户收藏商品列表展示、商品取消收藏、跳转商品详情
  - 状态: 已实现
  - 依赖: user.js(store), product.js(api)
  - 新增功能: 支持从收藏夹直接加入购物车
...
### 3. 购物车模块 (cart)
- **cart.js(api)**: 购物车相关API
  - 功能: 获取购物车、添加商品到购物车、更新商品数量、更新选中状态、批量操作、清空购物车
  - 状态: 已实现
  - 依赖: request.js(utils)

- **cart.js(store)**: 购物车状态管理
  - 功能: 购物车数据管理、商品数量调整、选中状态管理、批量操作
  - 状态: 已实现
  - 依赖: cart.js(api)
  - 计算属性: totalCount, selectedItems, allSelected
...
### 5. 组件模块 (components)
- **ProductCard.vue**: 商品卡片组件
  - 功能: 商品信息展示、收藏操作、加入购物车
  - 状态: 已实现
  - 依赖: 无
  - 新增功能: 加入购物车按钮、addToCart事件
...
- **cart.js**: 购物车相关接口
  ...
  - 主要接口: 
    ...
    - updateCartSelection - 更新购物车商品选中状态
    - batchUpdateCartSelection - 批量更新选中状态
...
- **product.js**: 商品相关接口
  ...
  - 状态: 已实现
  - 依赖: request.js(utils)
  - 新增接口: updateProductStatus(商品状态更新), updateProductStock(商品库存调整)
